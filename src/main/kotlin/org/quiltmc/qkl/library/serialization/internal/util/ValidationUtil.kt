...
internal fun validateKeyDescriptor(
    ...
    if (keyDescriptor.isNullable) {
        throw IllegalArgumentException("Regular maps do not support nullable keys")
    }
    ...
    if (keyKind !is PrimitiveKind && keyKind != SerialKind.ENUM && !carrierDescriptor.isCodec) {
        throw IllegalArgumentException(
...
private fun SerialDescriptor.carrierDescriptor(module: SerializersModule): SerialDescriptor {
    return when {
        kind == SerialKind.CONTEXTUAL ->
            module.getContextualDescriptor(this)?.carrierDescriptor(module) ?: this
